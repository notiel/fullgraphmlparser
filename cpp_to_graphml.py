...
#            ''' % (h.event_type, h.state_name, h.target_state_name, '\n'.join(h.statements)))
        ...
        if node.kind == clang.cindex.CursorKind.DEFAULT_STMT:
            # That's how subtree of such node is expected to look like
            # (generated by 'clang -Xclang -ast-dump <filename>.cpp')
    ...
    def _HandlersFromCompountStmt(self, compount_stmt_node):
        ...
        for node in compount_stmt_node.get_children():
            ...
            if node.kind == clang.cindex.CursorKind.BREAK_STMT:
                return [EventHandler(state_from=self.state_from, state_to=state_to, condition=condition,
                                     statements=statements, event_type="")]

            # It's a bit hacky:
            # 1) BINARY_OPERATOR doesn't necessarily mean equality operator.
            # Can be some weird dangling statement like 'a < Foo(5);'
            # 2) It would be better to check if it is a status_ assignment by checking first child,
            # which should be DECL_REF_EXPR node
...
class StateMachineWriter:
    ...
    def WriteToFile(self, filename: str, previous_filename: Optional[str] = None):
        ...
        create_graphml.get_constructor_fields_comment(self.graph, 'constructor_fields',
    ...
    def _OutputState(self, state, index_as_child, graph_parent, previous_states):
        ...
        full_node_name = (self.state_name_to_node_name[state.parent_state_name] +
                          ':' if state.parent_state_name != 'global'
        ...
        if previous_states_with_name:
            ...
            h = previous_states_with_name[0].height
            w = previous_states_with_name[0].width
            x0 = previous_states_with_name[0].x
            y0 = previous_states_with_name[0].y
        else:
